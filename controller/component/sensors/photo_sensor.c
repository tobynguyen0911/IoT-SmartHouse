...
const static char *TAG = "photo SENSOR";
...
cJSON * photo_create_state(int current_val)
{
    cJSON *state_root;
...
void init_photo_sensor(int pins[], device_def_t* device_def)
{
    ...
    device_def->state->device_data = photo_create_state(0);
...
{
    ESP_LOGI(TAG, "Entered Task: photo Detector");
    ...
    while (1) {
        ...
        int out_raw;
        int adc_pin = convert_analog_pin(device->pins[0]);
        adc_oneshot_read(adc1_handle, adc_pin, &out_raw);
        ESP_LOGI(TAG, "photo output: %d", out_raw);

        device->state->device_data = photo_create_state(out_raw);
...
void start_photo_task(int pins[], device_def_t * device) {
    TaskHandle_t xHandle = NULL;
    xTaskCreate( vTaskphotoSensor, "photo Sensor", STACK_SIZE, device, tskIDLE_PRIORITY, &xHandle );
...
static device_driver_t driver_funcs = {
    .driver_id = 13,
    .n_pins = 1,
    .init_func = &init_photo_sensor,
    .start_task = &start_photo_task,
    .functions = NULL,
    .num_functions = 0
};
