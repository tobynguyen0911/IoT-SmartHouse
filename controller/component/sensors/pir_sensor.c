...
cJSON * pir_create_state(int current_val)
{
    cJSON *state_root;
	state_root = cJSON_CreateArray();

    cJSON *state_element = cJSON_CreateObject();
    cJSON_AddStringToObject(state_element, "name", "pir");
...
void init_pir_sensor(int pins[], device_def_t* device_def)
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc1_handle, pins[0], &config));
    ...
    device_def->state->device_data = pir_create_state(0);
...
void vTaskpirSensor(void * pvParameters)
{
    ESP_LOGI(TAG, "Entered Task: pir Detector");
    ...
    while (1) {
        ...
        int out_raw;
        adc_oneshot_read(adc1_handle, device->pins[0], &out_raw);
        ESP_LOGI(TAG, "pir output: %d", out_raw);

        device->state->device_data = pir_create_state(out_raw);
...
void start_pir_task(int pins[], device_def_t * device) {
    TaskHandle_t xHandle = NULL;
    xTaskCreate( vTaskpirSensor, "pir Sensor", STACK_SIZE, device, tskIDLE_PRIORITY, &xHandle );
...
device_driver_t driver_funcs = {
    .driver_id = 6,
    .n_pins = 1,
    .init_func = &init_pir_sensor,
    .start_task = &start_pir_task,
    .functions = NULL,
    .num_functions = 0
};
