...
const static char *TAG = "SMOKE SENSOR";
...
cJSON * smoke_create_state(int current_val)
{
    cJSON *state_root;
	state_root = cJSON_CreateArray();

    cJSON *state_element = cJSON_CreateObject();
    cJSON_AddStringToObject(state_element, "name", "smoke");
...
void init_smoke_sensor(int pins[], device_def_t* device_def)
{
    ...
    device_def->state->device_data = smoke_create_state(0);
...
void vTaskSmokeSensor(void * pvParameters)
{
    ESP_LOGI(TAG, "Entered Task: Smoke Detector");
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "Smoke output: %d", out_raw);

        device->state->device_data = smoke_create_state(out_raw);
...
void start_smoke_task(int pins[], device_def_t * device) {
    TaskHandle_t xHandle = NULL;
    xTaskCreate( vTaskSmokeSensor, "Smoke Sensor", STACK_SIZE, device, tskIDLE_PRIORITY, &xHandle );
...
static device_driver_t driver_funcs = {
    .driver_id = 12,
    .n_pins = 1,
    .init_func = &init_smoke_sensor,
    .start_task = &start_smoke_task,
    .functions = NULL,
    .num_functions = 0
};
