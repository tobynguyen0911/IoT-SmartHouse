...
cJSON * pump_btn_create_state(int current_val)
{
    cJSON *state_root;
...
void set_pump_btn(device_def_t * device, cJSON * params)
{
    ...
    int new_pump_state = cJSON_IsTrue(params);
    gpio_set_level(device->pins[0], new_pump_state);

    cJSON_Delete(device->state->device_data);
    device->state->device_data = pump_btn_create_state(new_pump_state);
    device->state->updated = true;

    //taskEXIT_CRITICAL(device->state->dev_spinlock);
}

void init_pump_btn(int pins[], device_def_t* device_def)
{
    ...
    device_def->state->device_data = pump_btn_create_state(false);
...
void vPumpBtnTask(void * pvParameters)
{
...
void start_pump_btn_task(int pins[], device_def_t * device) {
    TaskHandle_t xHandle = NULL;
    xTaskCreate( vPumpBtnTask, "pump DRIVER", STACK_SIZE, device, tskIDLE_PRIORITY, &xHandle );
...
static device_driver_t driver_funcs = {
    .driver_id = 15,
    .n_pins = 1,
    .init_func = &init_pump_btn,
    .start_task = &start_pump_btn_task,
    .functions = NULL,
    .num_functions = 1
};
