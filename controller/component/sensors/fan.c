...
cJSON * fan_create_state(int current_val)
{
    cJSON *state_root;
...
void set_fan(device_def_t * device, cJSON * params)
{
    ...
    int new_fan_state = cJSON_IsTrue(params);
    gpio_set_level(device->pins[0], new_fan_state);

    cJSON_Delete(device->state->device_data);
    device->state->device_data = fan_create_state(new_fan_state);
...
void init_fan(int pins[], device_def_t* device_def)
{
    ...
    device_def->state->device_data = fan_create_state(false);
...
void start_fan_task(int pins[], device_def_t * device) {
    TaskHandle_t xHandle = NULL;
    xTaskCreate( vfanTask, "fan DRIVER", STACK_SIZE, device, tskIDLE_PRIORITY, &xHandle );
