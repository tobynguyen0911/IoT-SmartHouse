...
const static char *TAG = "soil SENSOR";
...
cJSON * soil_create_state(int current_val)
{
    cJSON *state_root;
...
void init_soil_sensor(int pins[], device_def_t* device_def)
{
    ...
    device_def->state->device_data = soil_create_state(0);
...
{
    ESP_LOGI(TAG, "Entered Task: soil Detector");
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "soil output: %d", out_raw);

        device->state->device_data = soil_create_state(out_raw);
...
void start_soil_task(int pins[], device_def_t * device) {
    TaskHandle_t xHandle = NULL;
    xTaskCreate( vTasksoilSensor, "soil Sensor", STACK_SIZE, device, tskIDLE_PRIORITY, &xHandle );
...
static device_driver_t driver_funcs = {
    .driver_id = 9,
    .n_pins = 1,
    .init_func = &init_soil_sensor,
    .start_task = &start_soil_task,
    .functions = NULL,
    .num_functions = 0
};
