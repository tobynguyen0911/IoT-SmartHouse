...
const static char *TAG = "steam SENSOR";
...
cJSON * steam_create_state(int current_val)
{
    cJSON *state_root;
...
void init_steam_sensor(int pins[], device_def_t* device_def)
{
    ...
    device_def->state->device_data = steam_create_state(0);
...
void vTasksteamSensor(void * pvParameters)
{
    ESP_LOGI(TAG, "Entered Task: steam Detector");
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "steam output: %d", out_raw);

        device->state->device_data = steam_create_state(out_raw);
...
void start_steam_task(int pins[], device_def_t * device) {
    TaskHandle_t xHandle = NULL;
    xTaskCreate( vTasksteamSensor, "steam Sensor", STACK_SIZE, device, tskIDLE_PRIORITY, &xHandle );
...
static device_driver_t driver_funcs = {
    .driver_id = 14,
    .n_pins = 1,
    .init_func = &init_steam_sensor,
    .start_task = &start_steam_task,
    .functions = NULL,
    .num_functions = 0
};
