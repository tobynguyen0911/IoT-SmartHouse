...
cJSON * buzzer_create_state(int current_val)
{
    cJSON *state_root;
...
void set_buzzer(device_def_t * device, cJSON * params)
{
    ...
    int new_buzzer_state = cJSON_IsTrue(params);
    gpio_set_level(device->pins[1], new_buzzer_state);

    cJSON_Delete(device->state->device_data);
    device->state->device_data = buzzer_create_state(new_buzzer_state);
    device->state->updated = true;

    //taskEXIT_CRITICAL(device->state->dev_spinlock);
}

void init_buzzer(int pins[], device_def_t* device_def)
{
    ...
    device_def->state->device_data = buzzer_create_state(false);
...
void start_buzzer_task(int pins[], device_def_t * device) {
    TaskHandle_t xHandle = NULL;
    xTaskCreate( vBuzzerTask, "BUZZER DRIVER", STACK_SIZE, device, tskIDLE_PRIORITY, &xHandle );
...
device_driver_t driver_funcs = {
    .driver_id = 1,
    .n_pins = 2,
    .init_func = &init_buzzer,
    .start_task = &start_buzzer_task,
    .functions = NULL,
    .num_functions = 1
};

driver_t * gen_buzzer_driver(int pins[])
{
    device_def_t def = {
